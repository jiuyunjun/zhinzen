...
- 已核对 `apps/android/google-services.json` 的项目与包名，复制到构建所需的
  `apps/android/app/google-services.json`。
- `local.properties` 缺少 Android Maps Key，已从工作区原有、包名匹配的
  `app-debug.apk` Manifest 恢复客户端 key，写入 Git 忽略的 `apps/android/local.properties`。
- 两类配置均不提交；更换机器仍需单独备份。客户端配置恢复不等于真机验证通过。
