...
- **后端**：`joinRoom` 进来时把 `expiresAt` 滑到 **now+30 天**（`SLIDING_ROOM_TTL_MS`）→ 日常会开就
  永不过期,长期不开才回收。已部署。
- **存储**：家人房间 id 存设备本地(web localStorage / android `RoomHistory.familyRoom` 偏好)。
- **设置入口**：地图顶栏的 **★ 星标**切换"当前房间=家人房间"。
- **自动进入**：启动时若设了家人房间 + 有名字 + 没有邀请链接/未在房间 → 自动 `joinRoom` 进地图
  (web `App.tsx` effect;android VM `init` 块,延迟执行让深链邀请优先)。失败(过期)则回退选择页。

**验证**：functions 部署;web build+deploy;android `assembleDebug` ✅ 并更新 APK。
注:30 天 TTL 下活跃房间的轨迹点会在 RTDB 留存更久(读取仍只取最近 24h);存储极小,暂可接受。
...
- 之前去掉是因为"按朝向分桶 RSSI"被距离变化主导 → 指反。本次**保留并修好**:
  `NearbyEstimator` 先用慢 EMA(α=0.05)估出 RSSI 基线(距离分量),用 **residual = smoothed −
  baseline** 按朝向分桶。残差反映的是**人体遮挡**(面朝对方时 2.4GHz 不被身体挡 → 残差更高),
  残差最大的朝向 ≈ 对方方向。需转身/走动覆盖 ≥4 个扇区且残差高出均值 ≥1.5dB 才给方向,
  否则显示趋势(↑/↓)+「走动几步以估计方向」。
- UI:`NearbyDirection` 有方向时显示箭头(相对当前朝向,连续角+动画),否则回退趋势字形。
  仍是**粗略提示**(精准方向是 UWB)。

**验证**：`assembleDebug` ✅。⚠️ 残差法仍需真机调(BASELINE_ALPHA/桶 margin 可能要按实测改)。
...
**新增 `nearby/NearbyEstimator`(纯逻辑)**:融合 BLE RSSI + 罗盘
- **RSSI 平滑**:EMA(α=0.25)。
- **粗距离**:路径损耗 `d=10^((TX−RSSI)/(10·N))`(TX=−59,N=2.5,clamp 0.3–60m),只显示**区间**
  (约 1–2 / 2–5 / 5–10 / 10–20 / 20+ 米),不报精确值(符合 §5.8)。
- **趋势**:最近 6s RSSI 斜率 → 正在靠近 ↑ / 正在远离 ↓ / 稳定。
- **IMU 方向估计**:按"用户面朝的罗盘方向"把 RSSI 分 8 桶做 EMA,信号最强的桶 = 大致方向
  (身体会挡 2.4GHz,面朝对方时 RSSI 更高 → 有物理依据)。需用户转身/走动采样多个方向才收敛,
  未收敛显示「走动几步以估计方向」。单手机无法从 BLE 直接测向,这是启发式近似(真测向要 UWB)。

**UI(成员详情)**:精度差(>30m)提示「室内 GPS 不准,用蓝牙找人」;显示方向箭头(指向最强信号
方向,连续角+动画防绕圈)+ 距离区间 + 信号格 + 「dBm · 趋势」。VM 在选中他人时把该 peer 的
RSSI+当前罗盘喂给 estimator → `nearbyEstimate`。

**验证**：`assembleDebug` ✅。⚠️ 需两台真机走动实测;TX/N、桶阈值、趋势灵敏度可能要按实测调。

---

## 2026-06-05 — 定位按钮移到右侧 FAB + BLE 显示具体信号强度 ✅（已编译）

**1) 定位自己按钮位置对齐网页**
- 关掉 Google 内置的右上角 my-location 按钮(`myLocationButtonEnabled=false`),在右侧 FAB
  列新增 **recenter FAB**(自绘十字准星图标),点击把相机动画到自己位置(zoom 16)。和网页
  右侧悬浮按钮一致。
- (Sony 确认是设备问题,LEGACY 渲染器保留作跨机型默认。)

**2) BLE 显示更具体的信号强度**
- 之前只有「较近/较远」。现在成员详情显示:**4 格信号条** + 「蓝牙 · 较近 · -68 dBm」
  (桶 + 实际 RSSI dBm)。`barsForRssi`:≥-55→4、≥-67→3、≥-78→2、else→1 格。
  仍不报精确米数(符合 design §5.8),但 dBm + 信号格更直观。
