...
- **底部两个显式相机模式 + 退出**:
  ...
  - 「查看两人」= **随自身朝向旋转的持续双人取景**，与「跟随自己」使用相同的 GPS 航向/
    罗盘来源；没有十秒倒计时，也不自动切回。随双方位置、朝向、屏幕尺寸持续更新布局。
    使用 Web Mercator 坐标，将两点差值旋转到当前屏幕方向后计算跨度；缩放不受跟随自己的
    zoom 13 下限限制（双人模式最低 0，最高 17.5）。跨日期变更线使用最短经度差。
    两人放在避开顶部 HUD、底部控件的安全区域中央，保留头像边距；转向时重新计算跨度，
    不调用会重置朝向的 fitBounds。缩小时立即容纳两人，放大时平滑，tilt 固定为 0。
    用户手势暂停自动相机，追踪目标仍保留；点任一视角按钮恢复并默认启用朝向跟随。
...
| 跟随自己时对方跑出视口 | 保持道路比例尺并提示在屏幕外；查看两人时持续缩放容纳两人 |
| 距离极远 | 跟随自己由速度决定比例尺；查看两人由旋转后的双点跨度决定比例尺 |
| 目标 `sharingLocation=false` / 被踢 / 离线 | **不退出**,继续追最后已知位置 + 显示年龄(见上"两条硬规则") |
| 手动拖动/缩放 | 两种视角都暂停自动相机但保留追踪；Web 区分程序写入缩放与用户缩放，Android 用 GESTURE |
